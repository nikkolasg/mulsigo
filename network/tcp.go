...
func (c *TCPConn) Receive() (m Message, e error) {
	defer func() {
		if err := recover(); err != nil {
			e = fmt.Errorf("tcp conn: Received message: %v\n", err)
			m = nil
		}
	}()
	...
	return c.Encoder.Unmarshal(buff)
}
...
func (c *TCPConn) receiveRaw() ([]byte, error) {
	...
	for read < total {
		...
		if _, err := buffer.Write(b[:n]); err != nil {
			log.Error("tcp conn: can't write to buffer:", err)
		}
		read += Size(n)
		b = b[n:]
	}
...
func (c *TCPConn) Send(msg Message) error {
	...
	buff, err := c.Encoder.Marshal(msg)
	if err != nil {
		return fmt.Errorf("tcp conn: %s error with message: %s", c.Remote(), err.Error())
	}
	return c.sendRaw(buff)
}
...
// If enc == nil, a MultiProtoEncoder is instantiated.
func NewTCPListener(addr Address, enc Encoder) (*TCPListener, error) {
	if addr.ConnType() != PlainTCP {
		return nil, errors.New("tcp listener: can't listen on non-tcp address")
	}
	if enc == nil {
		enc = NewMultiProtoEncoder()
	}
	...
	for i := 0; i < MaxRetryConnect; i++ {
		...
		if err == nil {
			...
			return nil, errors.New("tcp listener: " + err.Error())
		}
		time.Sleep(WaitRetry)
	}
...
func NewTCPHost(addr Address, e Encoder) (*TCPHost, error) {
	...
	var err error
	h.TCPListener, err = NewTCPListener(addr, e)
	return h, err
}
...
func (t *TCPHost) Connect(addr Address) (Conn, error) {
	...
	case PlainTCP:
		c, err := NewTCPConn(addr, t.TCPListener.Encoder)
		return c, err
	}
	return nil, fmt.Errorf("tcphost can't handle connection of type %s", addr.ConnType())
}
