...
func init() {
	RegisterMessage(50, BigMsg{})
	RegisterMessage(51, SimpleMessage{})
	RegisterMessage(53, statusMessage{})
}
...
func TestTCPConnReceiveRaw(t *testing.T) {
	// TODO transform that test for the encoder part
...
func TestTCPConn(t *testing.T) {
	...
	_, err := NewTCPConn(NewTCPAddress("127.0.0.1:7878"), nil)
	if err == nil {
	...
	// get addr
	listeningAddr := <-addr
	c, err := NewTCPConn(NewTCPAddress(listeningAddr), nil)
	require.Nil(t, err)
...
func TestTCPConnTimeout(t *testing.T) {
	...
	ln, err := NewTCPListener(addr, nil)
	if err != nil {
...
func TestTCPRouter(t *testing.T) {
	wrongAddr := NewLocalAddress("127.0.0.1:2000")
	_, err := NewTCPRouter(wrongAddr, nil)
	if err == nil {
		t.Fatal("Should not setup Router with local address")
	}
	addr := NewTCPAddress("127.0.0.1:2000")
	h1, err := NewTCPRouter(addr, nil)
	if err != nil {
		t.Fatal("Could not setup host")
	}
	defer h1.Stop()
	_, err = NewTCPRouter(addr, nil)
	if err == nil {
...
func TestTCPHostClose(t *testing.T) {
	...
	_, err = h2.Connect(NewTCPAddress(h3.addr.String()))
	if err != nil {
...
func (smp *simpleMessageProc) Process(from Address, msg Message) {
	if _, ok := msg.(*SimpleMessage); !ok {
		smp.t.Fatal("Wrong message")
	}
	sm := msg.(*SimpleMessage)
	smp.relay <- *sm
}
...
func (sp *simpleProcessor) Process(addr Address, msg Message) {
...
func sendrcvProc(from, to *Router) error {
	...
	// new processing
	to.RegisterProcessor(sp, statusMessage{})
	if err := from.Send(to.address, &statusMessage{true, 10}); err != nil {
		return err
	}
	var err error
	select {
	...
	to.RegisterProcessor(nil, statusMessage{})
	return err
}

func waitConnections(r *Router, addr Address) error {
	for i := 0; i < 10; i++ {
		c := r.connection(addr)
		if c != nil {
			return nil
	...
	return fmt.Errorf("Didn't see connection to %s in router", addr)
}
