...
func NewLocalRouter(addr Address, enc Encoder) (*Router, error) {
	return NewLocalRouterWithManager(defaultLocalManager, addr, enc)
}
...
func NewLocalRouterWithManager(l *LocalManager, addr Address, e Encoder) (*Router, error) {
	h, err := NewLocalHostWithManager(l, addr, e)
	if err != nil {
		return nil, err
	}
...
type LocalManager struct {
	...
	listening map[Address]infoListener
...
func NewLocalManager() *LocalManager {
	return &LocalManager{
		...
		listening: make(map[Address]infoListener),
	}
}
...
func (lm *LocalManager) setListening(addr Address, fn func(*LocalConn), enc Encoder) {
	lm.Lock()
	defer lm.Unlock()
	lm.listening[addr] = infoListener{fn, enc}
}
...
func (lm *LocalManager) connect(local, remote Address) (*LocalConn, error) {
	...
	outgoing := newLocalConn(lm, outEndpoint, incEndpoint, info.e)
	incoming := newLocalConn(lm, incEndpoint, outEndpoint, info.e)
...
// to use NewLocalConn.
func newLocalConn(lm *LocalManager, local, remote endpoint, e Encoder) *LocalConn {
	lc := &LocalConn{
		Encoder:       e,
		remote:        remote,
		local:         local,
...
// The encoder is determined by the local listener.
...
// If enc == nil, MultiProtoEncoder is instantiated.
func NewLocalListener(addr Address, enc Encoder) (*LocalListener, error) {
	return NewLocalListenerWithManager(defaultLocalManager, addr, enc)
}
...
func (ll *LocalListener) Listen(fn func(Conn)) error {
	...
	lfn := func(l *LocalConn) {
		fn(l)
	}
	ll.manager.setListening(ll.addr, lfn, ll.Encoder)
	ll.listening = true
	ll.Unlock()
...
func NewLocalHost(addr Address, e Encoder) (*LocalHost, error) {
	return NewLocalHostWithManager(defaultLocalManager, addr, e)
}
...
func NewLocalHostWithManager(lm *LocalManager, addr Address, e Encoder) (*LocalHost, error) {
	...
	var err error
	lh.LocalListener, err = NewLocalListenerWithManager(lm, addr, e)
	return lh, err
...
func (lh *LocalHost) Connect(addr Address) (Conn, error) {
	if addr.ConnType() != Local {
		return nil, fmt.Errorf("localhost: can't connect to non-Local address %s", addr)
	}
	var finalErr error
	for i := 0; i < MaxRetryConnect; i++ {
