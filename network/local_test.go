...
func TestLocalListener(t *testing.T) {
	addr := NewLocalAddress("127.0.0.1:2000")
	wrongAddr1 := NewAddress(PlainTCP, addr.NetworkAddress())
	listener, err := NewLocalListener(wrongAddr1, localEncoder)
	if err == nil {
		t.Error("Create listener with wrong address should fail")
	}
	defaultLocalManager.setListening(addr, func(c *LocalConn) {}, localEncoder)
...
func testConnListener(ctx *LocalManager, done chan error, listenA, connA Address, secret int) {
	addrEncoder := NewSingleProtoEncoder(&AddressTest{})
	listener, err := NewLocalListenerWithManager(ctx, listenA, addrEncoder)
	if err != nil {
		done <- err
		return
	}
	...
	h, err := NewLocalHostWithManager(ctx, connA, addrEncoder)
	if err != nil {
		done <- err
		return
	}
...
func testLocalConn(t *testing.T, a1, a2 Address) {
	addr1 := a1
	addr2 := a2
	simpleEncoder := NewSingleProtoEncoder(SimpleMessage{})
	listener, err := NewLocalListener(addr1, simpleEncoder)
	if err != nil {
		t.Fatal("Could not listen", err)
	}
	...
	go func() {
		...
		listener.Listen(func(c Conn) {
			...
			nm, err := c.Receive()
			assert.Nil(t, err)
			assert.Equal(t, 3, nm.(*SimpleMessage).I)
...
func TestDefaultLocalManager(t *testing.T) {
	defaultLocalManager.setListening(NewLocalAddress("127.0.0.1"), func(c *LocalConn) {}, localEncoder)
